...
public class GeneticAlgorithm {
     ...
     */
      ...
      public Population initPopulation(int populationSize, Paper paper, List<Question> qList) {
          this.db = qList;
          System.out.println(db+"FSDFSDFFFSDFSD***");
          Population population = new Population(populationSize);
          Map<String, Integer> typeCountMapping = paper.getTypeCountMapping();
       ...
       * @return
       */
      private List<Question> getAppointedQuestionList(List<Question> qList, String type, List<Integer> knowledgePoints) {
          List<Question> subList = new ArrayList<Question>();
          for(Question q:qList) {
              if(q.getType().equals(type) && knowledgePoints.containsAll(q.getKnowledgePoints())) {
                  subList.add(q);
              }
          }
          return subList;
          ...
          for(int populationIndex=0; populationIndex<population.size(); populationIndex++) {
              ...
              for(int geneIndex=0; geneIndex<mutateIndividual.getQuestionCount(); geneIndex++) {
                  if(Math.random() < adaptiveMutationRate && populationIndex > elitismCount) {
                      ...
                      String type = mutateGene.getType();
      ...
      private List<Question> getAppointedQuestionList(List<Question> qList, String type, List<Integer> kps, int mutateScore) {
          List<Question> subList = new ArrayList<Question>();
          for(Question q:qList) {
              if(q.getType() == type
